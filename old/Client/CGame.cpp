...
__declspec(noinline) void CGame::PickupsTick()
{
	AngleOfPickups += 0.1f;
	// a bit optimized
	float rw = cos(AngleOfPickups*0.1744f);
	float ry = sin(AngleOfPickups*0.1744f);
...
void CGame::ThrowAwayWeapon()
{
	...
	if ((g_CCore->GetLocalPlayer()->GetBase() + 0x4A0) == testPed)
	{
		...
		bsOut.Write(g_CCore->GetLocalPlayer()->GetOurID());
		bsOut.Write(wepId);
		g_CCore->GetNetwork()->SendServerMessage(&bsOut, IMMEDIATE_PRIORITY, RELIABLE, LHMP_NETCHAN_STATECHANGE);
	}
}
